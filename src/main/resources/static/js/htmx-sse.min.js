...
(function() {
  ...
  // Intercept and wrap dispatchEvent to prevent null reference errors
  // This catches the error at the source before HTMX's code fails
  var originalDispatchEvent = Element.prototype.dispatchEvent;
  Element.prototype.dispatchEvent = function(event) {
    try {
      return originalDispatchEvent.call(this, event);
    } catch (e) {
      if (e.message && e.message.indexOf('dispatchEvent') !== -1) {
        console.warn("Caught dispatchEvent error:", e.message);
        return false;
      }
      throw e;
    }
  ...
  // Global error handler to catch any remaining null dispatchEvent errors from HTMX
  var originalErrorHandler = window.onerror;
  window.onerror = function(msg, url, lineNo, columnNo, error) {
    // Catch HTMX's null dispatchEvent errors
    if (msg && typeof msg === 'string' && msg.indexOf('Cannot read properties of null') !== -1 && msg.indexOf('dispatchEvent') !== -1) {
      console.warn("Suppressed HTMX null dispatchEvent error:", msg);
      // Return true to suppress the error
      return true;
    }
    // Catch general null errors that might come from trigger()
    if (msg && typeof msg === 'string' && msg.indexOf('Cannot read properties of null') !== -1) {
      console.warn("Suppressed null reference error:", msg);
      return true;
    }
  ...
  // Also add a promise rejection handler for async errors
  window.addEventListener('unhandledrejection', function(event) {
    if (event.reason && event.reason.message && event.reason.message.indexOf('dispatchEvent') !== -1) {
      console.warn("Suppressed unhandled promise rejection with dispatchEvent:", event.reason);
