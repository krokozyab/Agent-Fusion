...
class PdfChunkerTest {
    ...
    fun `empty content returns empty list`() {
        ...
        val chunks = chunker.chunk("", "test.pdf", "pdf")
    ...
    fun `single paragraph creates one chunk with correct line numbers`() {
        ...
        val chunker = PdfChunker()
        val chunks = chunker.chunk(text, "test.pdf", "pdf").withoutRoot()
...
Second paragraph."""
        val chunker = PdfChunker()
        val chunks = chunker.chunk(text, "test.pdf", "pdf")
    ...
    fun `strategy metadata is correct`() {
        ...
        assertEquals("pdf", strategy.id)
        assertEquals("PDF Document Chunker", strategy.displayName)
    }

    @Test
    fun `summary field is null for pdf chunks`() {
